            ...
            for line_coord in multi_line:
                for index in range(len(line_coord)-1):
                    ...
                    if interpolate_interval > 0:
                        ...
                        point_feats = interpolate_line_segment(segment_geom, interpolate_interval)
...
def line_to_point_layer(list_feat, crs, layer_name="Point Layer", id_name="point_id", id_prefix='', filepath='memory'
):
    point_layer = QgsVectorLayer("Point?crs=" + crs, layer_name , filepath)
    point_layer_pr = point_layer.dataProvider()        
    point_layer_pr.addAttributes([QgsField("point_id", QVariant.String)])
...
def line_feature_list_to_layer(list_feat, crs, layer_name="Line Layer", id_name='line_id', filepath = 'memory'):
...
def create_delaunay_triangulation(merged_pt_layer):
    ...
    dt_layer_pr.addAttributes([QgsField("delaunayID", QVariant.Int),
...
def valid_delaunay_triangulation(dt_layer, list_feature_a, list_feature_b):
    # Valid Delaunay
    valid_delaunay_request = QgsFeatureRequest(QgsExpression(f"\"valid_triangle\" = 'valid'"))
    valid_delaunay_list = [feat.geometry() for feat in dt_layer.getFeatures(valid_delaunay_request)]
    # create multipart
...
def create_median_line_opposite(vd_layer, list_feature_a, list_feature_b, crs):
    ...
    # convert to line
    dissolve_voronoi_a_line = QgsGeometry.fromPolylineXY(dissolve_voronoi_a.asPolygon()[0])
    dissolve_voronoi_b_line = QgsGeometry.fromPolylineXY(dissolve_voronoi_b.asPolygon()[0])
...
def create_median_line_adjacent(vd_layer, list_feature_a, list_feature_b, crs):
    ...
    if len(list_pt_of_intersection) == 1:
        ...
        unique_list_pt_of_intersection = [QgsGeometry().fromWkt(wkt_pt) for wkt_pt in list(set(wkt_list_pt_of_intersection))]
    ...
    for line in list_median_line_single:
        ...
        geom_line = QgsGeometry().fromPolylineXY([closest_vertex[0], meeting_point.asPoint()])
...
def create_equidistant_point(median_layer):
    equidistant_pt_layer = QgsVectorLayer("Point?crs=" + median_layer.crs().authid(), "Equidistant Point", "memory")
...
def create_construction_line(vd_layer, equidistant_pt_layer, merged_pt_layer):
    cl_layer = QgsVectorLayer("Linestring?crs=" + vd_layer.crs().authid(), "Construction Line", "memory")
    ...
    for feat in equidistant_pt_layer.getFeatures():
        ...
        for f in req_feat:
            if f.geometry().intersects(feat.geometry()):
                pid = f['point_id']
                list_pid.append(pid)
                pt_feat = [feat for feat in merged_pt_layer.getFeatures(QgsFeatureRequest(QgsExpression(f"\"point_id\" = '{pid}'")))]
                if len(pt_feat) == 1:
                    list_pt.append(pt_feat[0].geometry().asPoint())
        if len(list_pt)==3:
            eq_pt = feat.geometry().asPoint()
            # create multilinestring feature
            cl_geom = QgsGeometry().fromMultiPolylineXY([[QgsPointXY(eq_pt), QgsPointXY(list_pt[0])], 
